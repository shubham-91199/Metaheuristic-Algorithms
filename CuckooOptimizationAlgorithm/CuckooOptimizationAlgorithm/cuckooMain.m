...
% [fitness fitness1 y] = fitnessFunc(x)
costFunction='fitnessFunc';
% F6 +/-5.12
[Npar VarLow VarHigh]=bounds();
npar  = Npar;          % number of optimization variables
varLo = VarLow;         % Lower  band of parameter
varHi =  VarHigh;         % Higher band of parameter


%% Set COA parameters

numCuckooS = 50;            % number of initial population
...
knnClusterNum = 2;          % number of clusters that we want to make
motionCoeff = 9;            % Lambda variable in COA paper, default=2
accuracy = -inf;            % How much accuracy in answer is needed
maxNumOfCuckoos = 200;      % maximum number of cuckoos that can live at the same time
radiusCoeff = 50;           % Control parameter of egg laying
...
globalMaxProfit = -feval(costFunction,globalBestCuckoo);
profitVector = [];
while ( (iteration <= maxIter) && (-maxProfit > accuracy) )
    radiusCoeff = 0.99 * radiusCoeff;
    ...
    cuckooPop{end-1}.center = rand(1,npar).*globalBestCuckoo;
